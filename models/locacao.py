...
class Locacao:
  def __init__(self, id, entrega, confirmado, id_cliente, id_filme):
    self.__id = id
  ...
  def __eq__(self, x):
    if self.__id == x.__id and self.__entrega == x.__entrega and self.__confirmado == x.__confirmado and self.__id_cliente == x.__id_cliente and self.__id_filme == x.__id_filme:
      return True
    return False

  def __str__(self):
    return f"{self.__id} - {self.__entrega.strftime('%d/%m/%Y %H:%M')} - {self.__confirmado} - {self.__id_cliente} - {self.__id_filme}"
...
class NLocacao:
  ...
  def listar_nao_confirmados(cls):
    ...
    for aux in cls.__locacoes:
      if not aux.__confirmado and aux.__entrega > hoje:
    ...
    try:
      with open("locacoes.json", mode="r") as arquivo:
        ...
        for obj in locacoes_json:
          aux = Locacao(
            obj["id"],
            datetime.datetime.strptime(obj["entrega"], "%d/%m/%Y %H:%M"),
            obj["confirmado"], obj["id_cliente"], obj["id_filme"])
